...
def test_forget_validator():
    ...
    mocked_update.message.text = f"/forget {public_key}"
    ...
    bot_ser.forget_validator(mocked_update, mocked_context)
    
    update_validator = validate.find_one_by_public_key(public_key)
    assert not user_id in update_validator['list_of_user_id_for_alarm']
